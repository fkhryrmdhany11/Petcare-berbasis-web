document.addEventListener('DOMContentLoaded', () => {
    // 1. Injeksi CSS Form Booking
    document.head.insertAdjacentHTML('beforeend', `
        <style>
            .booking-form { background: #f8fafc; border: 2px solid var(--button, #BDE8F5); border-radius: 20px; padding: 25px; margin-top: 20px; }
            .form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px; }
            .form-group { display: flex; flex-direction: column; gap: 5px; }
            .full-width { grid-column: span 2; }
            .form-group label { font-size: 14px; font-weight: 600; color: var(--primary, #0F2854); }
            .form-group input, .form-group select { width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 10px; }
            .btn-submit { width: 100%; background: var(--primary, #0F2854); color: #fff; padding: 12px; border: none; border-radius: 10px; cursor: pointer; }
            .badge-status { padding: 4px 10px; border-radius: 10px; font-size: 12px; background: #fef08a; color: #854d0e; }
        </style>
    `);

    // 2. Injeksi HTML Form Booking
                ...
                <form id="bookingForm" class="booking-form">
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            <label>Layanan</label>
                            <select id="sType">
                                <option>Booking Jadwal Periksa</option>
                                <option>Jadwalkan Vaksin</option>
                                <option>Panggil Dokter Ke Rumah</option>
                            </select>
                        </div>
    ...
    // Format Kartu Pelanggan (Dinamis membaca status dari Admin)
    const renderCard = (d) => {
        ...
        const tanggalRapi = d.bd ? new Date(d.bd).toLocaleString('id-ID', {
        ...
        <div class="appointment-card">
            ...
            <div class="appt-time">
                ...
                <p class="time-value">${tanggalRapi}</p>
            </div>
            ${tombolAksi}
        </div>`;
    };
    ...
    // Otomatis isi form saat card layanan diklik
    document.querySelectorAll('.cards-grid .card').forEach(c => {
        c.onclick = () => {
            const serviceTitle = c.querySelector('h3')?.innerText;
            if (serviceTitle) document.getElementById('sType').value = serviceTitle;
    ...
    if (form) {
        form.onsubmit = (e) => {
            e.preventDefault();
            const newData = {
                on: document.getElementById('oName').value,
                pn: document.getElementById('pName').value,
                pt: document.getElementById('pType').value,
                st: document.getElementById('sType').value,
            ...
            if (container) container.insertAdjacentHTML('afterbegin', renderCard(newData));
